...
from .utils import ensure_dir, safe_filename, to_camel_case, write_text_if_changed
...
class BaseToolWriter:
    ...
    def _function_signature(self) -> str:
        params = [PythonParameter(param, structured=param.name in self.tool.structured_params, list_normalizer=self.tool.list_params.get(param.name)) for param in self.tool.parameters]
        fragments: List[str] = []
