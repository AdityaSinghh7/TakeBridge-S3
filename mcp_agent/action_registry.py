...
def _remove_registered_actions(aci_class: Type) -> None:
    """Remove all previously attached MCP action shims from the ACI class."""
    for action_name in list(_REGISTERED_ACTION_NAMES):
        if hasattr(aci_class, action_name):
            try:
                delattr(aci_class, action_name)
            except Exception:
                pass
    _REGISTERED_ACTION_NAMES.clear()


def sync_registered_actions(user_id: str, aci_class: Type) -> None:
    """Synchronize MCP action shims on the ACI with currently available tools.

    Args:
        user_id: The user ID for which to sync actions
        aci_class: The ACI class on which to register action methods
    """
    active_user = normalize_user_id(user_id)
    _remove_registered_actions(aci_class)
    registered: List[Tuple[str, str]] = []
    for provider, fn in iter_available_action_functions(user_id=active_user):
        setattr(aci_class, fn.__name__, fn)
