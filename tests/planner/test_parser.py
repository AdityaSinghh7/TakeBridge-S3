...
def test_parse_planner_command_valid_finish():
    command = parse_planner_command('{"type":"finish","summary":"done"}')
...
def test_parse_planner_command_requires_tool_fields():
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"tool","tool":"send"}')
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"tool","provider":"slack"}')
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"tool","provider":"slack","tool":"send","payload":123}')


def test_parse_planner_command_validates_sandbox_and_search():
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"sandbox","code":""}')
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"search","query":""}')
    with pytest.raises(ValueError):
        parse_planner_command('{"type":"search","query":"gmail","limit":200}')
