...
Thin wrapper around the Baseten Chat Completions API (OpenAI-compatible).
"""
...
try:
    ...
    class _MissingOpenAI:  # pragma: no cover
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "The 'openai' package is required to instantiate BasetenClient. "
                "Install openai>=1.0 to enable Baseten calls."
            )
...
def _extract_text_from_content(content: Any) -> tuple[str, bool]:
    ...
    if isinstance(content, list):
        ...
        has_image = False
        for item in content:
            text, item_has_image = _extract_text_from_content(item)
            if text:
                chunks.append(text)
            if item_has_image:
                has_image = True
        return "".join(chunks), has_image
...
def _normalize_messages_for_chat(messages: Iterable[Message]) -> tuple[List[Dict[str, Any]], bool]:
    ...
    for message in messages:
        ...
        text, message_has_image = _extract_text_from_content(content)
...
def _coerce_input_to_messages(
    ...
    if isinstance(input_value, str):
        return [{"role": "user", "content": input_value}], False
    if isinstance(input_value, dict):
        if "role" in input_value:
            return _normalize_messages_for_chat([input_value])
        text, has_image = _extract_text_from_content(input_value)
        return [{"role": "user", "content": text}], has_image
    if isinstance(input_value, list):
        if all(isinstance(item, dict) and "role" in item for item in input_value):
            return _normalize_messages_for_chat(input_value)
        text, has_image = _extract_text_from_content(input_value)
        return [{"role": "user", "content": text}], has_image
    return [{"role": "user", "content": str(input_value)}], False
...
class BasetenClient:
    """Reusable Baseten client for issuing Chat Completions API calls with sane defaults."""
    ...
    def as_langchain_chat_model(
        ...
        **kwargs: Any,
        ...
        """
        Build a langchain_openai.ChatOpenAI model for Baseten (OpenAI-compatible).
        """
        try:
            from langchain_openai import ChatOpenAI
        except Exception as exc:  # pragma: no cover - optional dependency
            raise RuntimeError(
                "The 'langchain-openai' package is required to use Baseten via LangChain."
            ) from exc

        resolved_model = model or self._default_model
        resolved_api_key = api_key or self._api_key or os.getenv("BASETEN_API_KEY")
        resolved_base_url = (
            base_url
            or self._base_url
            or os.getenv("BASETEN_BASE_URL")
    ...
    def create_response(
        ...
        **kwargs: Any,
        ...
        """
        Issue a Chat Completions API request to Baseten.
        """
        ...
        if input is not None:
            normalized_messages, has_image = _coerce_input_to_messages(input)
        else:
            normalized_messages, has_image = _normalize_messages_for_chat(messages or [])

        if has_image:
            raise ValueError("Baseten does not support image content in chat completions.")
    ...
    def stream_response(
        ...
        **kwargs: Any,
        ...
        """
        Stream chat completions and return a synthesized response object.
        """
